...
> F1-score 기준 비교 결과, 
> **HistGradientBoosting 모델**이 대부분의 게임에서 가장 안정적인 이탈 예측 성능을 보였다.
...
CSV 기반 관찰한 결과 1차 베이스라인 모델에서 일부 게임은 실제 이탈 비율(churn_rate)보다
모델의 이탈 예측 비율(pos_rate)이 과도하게 높게 나타남

- 예시 (learning_first.csv)

`appid` = 3405690 `churn_rate` = 0.1740 `test_pos_rate` = 0.4229 `pos_gap` ≈ +0.249

해당 케이스는 Recall은 높지만 Precision이 낮아,
결과적으로 F1-score가 기대만큼 개선되지 않거나 오히려 하락하는 현상으로 이어짐
...
def best_threshold_by_f1(y_true, proba, thr_grid=THR_GRID):
    ...
    return best_thr
```
`pos_rate`는 “모델이 맞추는 것처럼 보이지만 실제로는 1을 과도하게 예측하는 문제”를
조기에 식별하는 핵심 지표로 활용됨
...
```
이탈률이 매우 낮은 게임에서는 과도한 synthetic sample 생성을 제한,
실제로 3차 학습에서 **test_pos_rate 감소(과예측 완화)**로 이어짐

> **고정 threshold(0.5)**는 게임별 이탈 예측에 적합하지 않음
> Threshold 최적화 + pos_rate 모니터링이 과예측 방지의 핵심
> SMOTE는 만능 해결책이 아니며,
> 데이터 특성에 맞게 보수적으로 적용해야 효과적
